<!-- Paragraph 1 -->
<p>
    All models in the database ”Molecular Models of the Boltzmann-Zuse Society” consist of Lennard-Jones 12-6 interaction sites and a variation of point charges, point-dipoles and point quadrupoles. The last two are computationally much cheaper compared to the corresponding conﬁguration of point charges. 
...
<div class="def_content">
    ...
    <p>
        For <b>unlike interactions </b> – the interaction of two sites with different $\varepsilon$ and/ or $\sigma$ – mixing rules can be applied. The parameters $\eta_{kl}$ and $\xi_{kl}$ in eq. (2) and (3) are used to correct the binary interaction parameters of the components $k$ and $l$ (if necessary). $\eta_{kl}$ and $\xi_{kl}$ are mostly a constant for a certain mixture. The extensive study of the inﬂuence of diﬀerent mixing rules by Schnabel et al. <a href="reflist.php" target="_blank"> [Schnabel, 2007_C] </a> showed, that mixture bubble densities are accurately obtained using the arithmetic mean of the two size parameters $\sigma_k,~\sigma_l$ as proposed by the Lorentz combining rule (3). That results in $\eta_{kl}=1$ being a very accurate description of the unlike size parameter. The vapor pressure turns out to be dependent on both unlike Lennard-Jones parameters. It was therefore recommended by Schnabel et al. to adjust the unlike Lennard-Jones energy parameter to the vapour pressure.
    </p>
    <div class="normen_equa">
        \begin{equation}
...
<div class="def_content">
    <p>
        Point charges are ﬁrst order electrostatic interaction sites. These sites are indicated in the database with an ’e’.
...
<div class="def_content">
    <p>
        A point dipole describes the electrostatic ﬁeld of two point charges with equal magnitude, but opposite sign at a mutual distance $a\to 0$. Point dipole sites are labelled throughout the database with a ’d’. The magnitude of a dipole moment is deﬁned by $\mu=qa$. The electrostatic interaction between two point dipoles with the moments $\mu_i$ and $\mu_j$ at a distance $r_{ij}$ is given by:
    </p>
    <div class="normen_equa">
        \begin{equation}
...
<div class="def_content">
    <p>
        A linear point quadrupole describes the electrostatic ﬁeld (sf. ﬁg. 4) induced either by two collinear point dipoles with the same moment, but opposite orientation at a distance $a\to 0$ or four point charges. Point quadrupole sites are labelled as ’q’ in the database. The magnitude of a point-quadrupole $Q$ is deﬁned as $Q = 2qd^2$, where $q$ is the magnitude of the three similar charges and $d$ their distance. The interaction potential is given by:
    </p>
    <div class="normen_equa">
        
$$\begin{eqnarray}         
u_{ij}^\mathrm{qq}(r_{ij},\theta_i,\theta_j,\phi_{ij},Q_i,Q_j) = \frac{1}{4\pi\epsilon_0}\frac{3}{4}\frac{Q_iQ_j}{r^5_{ij}}\left[1-5((\cos(\theta_i)^2+\cos(\theta_i)^2)  \\[5pt]
...
\end{eqnarray}$$
    ...
    <p>
        where the angles $\theta_i$, $\theta_j$ and $\phi_{ij}$ indicate the relative angular orientation of the two point quadrupoles.
    </p>
