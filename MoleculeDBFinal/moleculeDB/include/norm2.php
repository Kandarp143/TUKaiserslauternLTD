<p>The force ﬁelds shown in the database are displayed in the following units system, cf. Tab.2.
</p>
<table>
    <tr style="border-bottom: solid 1px grey;">
        <td><b></b></td>
        <td><b>symbol</b></td>
        <td><b>unit</b></td>
        <td><b></b></td>
    </tr>
    <tr>
        <td>length</td>
        <td><span class="mjxi">&sigma;</span></td>
        <td>&#8491;</td>
        <td>&#8491;ngstr&ouml;m</td>
    </tr>
    <tr>
        <td>energy</td>
        <td><span class="mjxi"><span>&epsilon;/k<sub>B</sub></span></span></td>
        <td><span>&#8490;</span></td>
        <td>Kelvin</td>
    </tr>
    <tr>
        <td>charge</td>
        <td><span class="mjxi">q</span></td>
        <td>e</td>
        <td>Elementary charge</td>
    </tr>
    <tr>
        <td>dipole</td>
        <td><span class="mjxi">&mu;</span></td>
        <td>D</td>
        <td>Debye</td>
    </tr>
    <tr>
        <td>angle</td>
        <td><span >$\theta$</span>,$\phi$</td>
        <td><span>&#xb0;</span></td>
        <td>degree</td>
    </tr>
    <tr>
        <td>mass</td>
        <td><span class="mjxi">M</span></td>
        <td>g/mol</td>
        <td>molar mass</td>
    </tr>
    <tr>
        <td>quadrupole</td>
        <td><span class="mjxi">Q</span></td>
        <td>D<span>&#8491;</span></td>
        <td>Buckingham</td>
    </tr>
</table>
	<p> </p>